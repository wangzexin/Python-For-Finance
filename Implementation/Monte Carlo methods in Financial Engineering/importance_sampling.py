...
def importance_sampling_european_call(strikePrice):
    # parameters
    sigma = 0.25;   T = 1.0;   I = 50000;   S0 = 100;   r = 0.05;  A = 0.005
    # pilot simulation
    ...
    stockPrices = S0*np.exp((r-sigma**2/2)*T+sigma*np.sqrt(T)*standardNormals)
    optionPayoffs = np.maximum(stockPrices - strikePrice, 0)
    newPayoffs = optionPayoffs * np.exp(A * standardNormals - A ** 2 * T / 2)
